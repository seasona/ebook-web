...
static bool xml2json_has_digits_only(const char *input, bool *hasDecimal) {
    if (input == nullptr)
        return false;  // treat empty input as a string (probably will be an
                       // empty string)
...
inline void xml2json_to_array_form(const char *name, rapidjson::Value &jsvalue,
                            rapidjson::Value &jsvalue_chd,
                            rapidjson::Document::AllocatorType &allocator) {
    rapidjson::Value jsvalue_target;  // target to do some operation
    rapidjson::Value jn;  // this is a must, partially because of the latest
...
inline void xml2json_add_attributes(rapidxml::xml_node<> *xmlnode,
    ...
    for (myattr = xmlnode->first_attribute(); myattr;
        ...
        jn.SetString(
            (std::string(xml2json_attribute_name_prefix) + myattr->name())
                .c_str(),
            allocator);
        ...
        if (xml2json_numeric_support == false) {
            ...
            bool hasDecimal;
            if (xml2json_has_digits_only(myattr->value(), &hasDecimal) ==
                false) {
                jv.SetString(myattr->value(), allocator);
            } else {
                if (hasDecimal) {
...
inline void xml2json_traverse_node(rapidxml::xml_node<> *xmlnode,
                            rapidjson::Value &jsvalue,
                            rapidjson::Document::AllocatorType &allocator) {
    ...
    if ((xmlnode->type() == rapidxml::node_data ||
        ...
        if (xmlnode->first_attribute()) {
            ...
            if (!xmlnode->first_node()) {
                ...
                jsvalue.SetNull();
                return;
            } else if (xmlnode->first_node()->type() == rapidxml::node_data &&
                       count_children(xmlnode) == 1) {
                // case: <e>text</e>
                if (xml2json_numeric_support == false) {
                    jsvalue.SetString(
                    ...
                    bool hasDecimal;
                    if (xml2json_has_digits_only(xmlnode->first_node()->value(),
                                                 &hasDecimal) == false) {
                        jsvalue.SetString(rapidjson::StringRef(
                                              xmlnode->first_node()->value()),
                                          allocator);
                    } else {
                        if (hasDecimal) {
                            double value = std::strtod(
                                xmlnode->first_node()->value(), nullptr);
                            jsvalue.SetDouble(value);
                        } else {
                            long int value = std::strtol(
                                xmlnode->first_node()->value(), nullptr, 0);
        ...
        if (xmlnode->first_node()) {
            ...
            for (xmlnode_chd = xmlnode->first_node(); xmlnode_chd;
                ...
                rapidjson::Value jn, jv;
                if (xmlnode_chd->type() == rapidxml::node_data ||
                ...
                if (name_count[current_name] > 1 && name_ptr)
                    xml2json_to_array_form(name_ptr, jsvalue, jsvalue_chd,
                                           allocator);
                else {
...
inline std::string xml2json(const char *xml_str) {
    ...
    for (xmlnode_chd = xml_doc->first_node(); xmlnode_chd;
        ...
        xml2json_traverse_node(xmlnode_chd, jsvalue_chd, allocator);
        js_doc.AddMember(rapidjson::StringRef(xmlnode_chd->name()), jsvalue_chd,
                         allocator);
    }
